...
// addEvents() takes a CSV of events in the following format, and adds them 
// to a calendar:
// 2/25/2014,2/25/2014,1100,1115,Event A
// 2/25/2014,2/25/2014,1200,1230,Event B

// The addEvents() method needs to create a Day if one does not exist for a 
// particular
...
// getDay() returns the pointer in dayList of
...
// This function returns a pointer to the Day so other functions can insert 
// events into that Day.
...
for (int j = dayCount; j > index; j--){
dayList[j] = dayList[j-1];
}
dayList[index] = *newDay;
dayCount++;
return &dayList[index];
...
startDateString = convertDateSt(oneLine.substr(0,8),
startTimeString);
}
...
endDateString = convertDateSt(oneLine.substr(0,8),
endTimeString);
}
...
if (dateStringToStruct(startDateString).year>=
startYear) {
// it is, so we use this event
csvStream << startDateString << "," << 
endDateString << "," <<
startTimeString << "," << 
endTimeString << "," <<
oneLine << "\n";
...
if (timeNum >= 1900) { 
// we went back one day, because of the GMT timezone
d.day--;
if (d.day==-1) {
d.month--;
...
return dateStringToStruct(csvLine.substr(index,csvLine.find(",",
index+1)));
}
...
istringstream(csvLine.substr(index+1,csvLine.find(",",index+1))) >> 
temp;
e.setStartTime(temp);
